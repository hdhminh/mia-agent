...
from agent.skills.media_service.schemas import MediaRequest, MediaResult
...
def _error_result(*, tool: str, exc: Exception, request_id: str = "", chat_id: str = "", file_name: str = "") -> MediaResult:
    ...
    envelope = build_exception_envelope(
        ...
        user_message=str(exc) if category == "validation" else t("skills.media_unprocessed", default="Mia chưa xử lý được tệp này. Bạn thử lại nhé."),
        retryable=False,
        source=ErrorSource(
            layer="api",
            component="media",
...
def tts_speak(request: Request, payload: MediaRequest) -> Response:
    ...
    try:
        ...
        envelope = build_exception_envelope(
            ...
            user_message=str(exc) if isinstance(exc, ValueError) else t("skills.tts_unprocessed", default="Mia chưa tạo được audio này. Bạn thử lại nhé."),
            retryable=False,
            source=ErrorSource(
                layer="api",
                component="media",
