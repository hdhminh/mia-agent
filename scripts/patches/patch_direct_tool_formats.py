...
INTEGRATIONS = ROOT / "execution/integrations"
SIMPLE = INTEGRATIONS / "simple"
...
const parts = [`Mia vừa lấy giá vàng SJC 9999 mới nhất cho ${owner}.`];
...
if (results.length > 0) {
  ...
  text += `\\nMia chưa thấy kết quả nào đủ rõ. ${owner} thử đổi từ khóa cụ thể hơn nhé.`;
}
...
if (finalItems.length === 0) {
  return [{ json: { text: `Hiện chưa có bài mới nổi bật để Mia gửi ${owner}.`, chatId } }];
}

const topicLine = [...new Set(finalItems.map((item) => item.topic))].slice(0, 2).join(', ');
let text = topicLine ? `Mia điểm nhanh vài tin nổi bật về ${topicLine}:` : `Mia điểm nhanh vài tin nổi bật cho ${owner}:`;
...
return [{ json: { text: text.trim(), chatId } }];""",
    ...
    INTEGRATIONS / "google/gmail/workflow_sub_google_gmail_list_inbox.json": {
...
if (rows.length === 0) {
  text = `Hộp thư của ${owner} hiện không có email mới đáng chú ý.`;
} else {
  text = `${owner} đang có ${rows.length} email đáng chú ý gần đây:`;
  const links = [];
...
return [{ json: { chatId: source.chatId, text: text.trim() } }];""",
    ...
    INTEGRATIONS / "google/calendar/workflow_sub_google_calendar_list_today.json": {
...
let text = '';
if (rows.length === 0) {
  text = `Hôm nay ${owner} chưa có lịch nào cả.`;
} else {
  text = `Hôm nay ${owner} có mấy lịch như sau:`;
  const links = [];
  for (let i = 0; i < rows.length; i++) {
    const event = rows[i];
...
return [{ json: { chatId: source.chatId, text: text.trim() } }];""",
    ...
    INTEGRATIONS / "google/calendar/workflow_sub_google_calendar_list_tomorrow.json": {
        "Format Danh Sach Lich": """function toDate(value) {
  const date = new Date(value);
  return Number.isNaN(date.getTime()) ? null : date;
}
...
let text = '';
if (rows.length === 0) {
  text = `Ngày mai ${owner} chưa có lịch nào cả.`;
} else {
  text = `Ngày mai ${owner} có mấy lịch như sau:`;
  const links = [];
  for (let i = 0; i < rows.length; i++) {
    const event = rows[i];
    const title = event.summary || 'Không có tiêu đề';
...
return [{ json: { chatId: source.chatId, text: text.trim() } }];""",
    ...
    INTEGRATIONS / "google/docs/workflow_sub_google_docs_search_doc.json": {
...
return [{ json: { chatId: source.chatId, text: text.trim() } }];""",
    ...
    INTEGRATIONS / "google/drive/workflow_sub_google_drive_list_files.json": {
...
return [{ json: { chatId: source.chatId, text: text.trim() } }];""",
    ...
    INTEGRATIONS / "google/drive/workflow_sub_google_drive_search_file.json": {
...
return [{ json: { chatId: source.chatId, text: text.trim() } }];""",
    ...
    INTEGRATIONS / "google/sheets/workflow_sub_google_sheets_search_sheet.json": {
        "Format Action": """const source = $('Prepare Action').item.json || {};
const rows = $input.all().map((item) => item.json || {}).filter((item) => item.id || item.name).slice(0, 3);
const query = source.query || '';
...
return [{ json: { chatId: source.chatId, text: text.trim() } }];""",
    ...
    INTEGRATIONS / "shortlink/workflow_shortlink_create.json": {
...
let text = `Mia đã rút gọn link xong rồi ${owner}.`;
if (shortUrl) text += `\\nLink ngắn: ${shortUrl}`;
