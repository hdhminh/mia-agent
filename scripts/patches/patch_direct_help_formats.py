...
PATCHES = {
    INTEGRATIONS / "google/calendar/workflow_sub_google_calendar_help.json": {
...
const text = [
  `Mia có thể hỗ trợ Google Calendar cho ${owner} ở các việc như xem lịch, tìm sự kiện, tạo lịch, huỷ lịch và kiểm tra giờ rảnh.`,
  '',
  `${owner} thử vài câu kiểu này là được:`,
  '- lịch hôm nay',
  '- lịch ngày mai',
  '- tìm lịch họp khách',
...
return [{ json: { chatId, text } }];""",
    ...
    INTEGRATIONS / "google/gmail/workflow_sub_google_gmail_help.json": {
...
const text = [
  `Mia có thể hỗ trợ Gmail cho ${owner} theo vài việc chính như xem hộp thư, đọc mail, tìm mail, soạn nháp, gửi mail và trả lời mail.`,
  '',
  `${owner} có thể thử ngay mấy câu như:`,
  '- xem mail',
  '- đọc mail từ Google',
  '- tìm mail hóa đơn',
...
return [{ json: { chatId, text } }];""",
    ...
    INTEGRATIONS / "google/drive/workflow_sub_google_drive_help.json": {
...
const text = [
  `Google Drive hiện cho phép Mia hỗ trợ ${owner} ở các việc như xem file gần đây, tìm file hoặc thư mục, xem thông tin file, tạo folder, tạo file, chia sẻ, đổi tên, di chuyển và xoá file.`,
  '',
  `${owner} có thể bắt đầu bằng mấy câu như:`,
  '- xem file drive',
  '- tìm file hợp đồng',
...
return [{ json: { chatId, text } }];""",
    ...
    INTEGRATIONS / "google/docs/workflow_sub_google_docs_help.json": {
...
const text = [
  `Với Google Docs, Mia có thể giúp ${owner} tạo tài liệu, đọc nội dung, thêm nội dung, tìm tài liệu và xoá tài liệu khi cần.`,
  '',
...
return [{ json: { chatId, text } }];""",
    ...
    INTEGRATIONS / "google/sheets/workflow_sub_google_sheets_help.json": {
