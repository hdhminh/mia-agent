...
from agent.persona.followup_cues import (
...
class GitHubHandler:
    ...
    def _limit_text_to_bullets(self, text: str, max_bullets: int = 6) -> str:
        ...
        limited.append(
            t(
                "skills.github_limited_suffix",
                default="- Còn {count} ý nữa đã được lược bớt; nếu anh muốn, Mia sẽ bóc tiếp chi tiết từng file hoặc lệnh.",
    ...
    def _summarize_github_repo_analysis(
        ...
        command_text = "\n".join(f"- {line}" for line in command_lines) if command_lines else t("skills.github_no_commands", default="không thấy lệnh rõ ràng trong dữ liệu")
        result, provider_used = self.service._invoke_model_with_fallback(
            ...
            [
                SystemMessage(
                    content=t(
                        "skills.github_system_analysis_prompt",
                        default=(
                ...
                HumanMessage(
                    content=t(
                        "skills.github_human_analysis_prompt",
                        default=(
                            "Câu hỏi của người dùng:\n{user_query}\n\n"
                            "Repo đang phân tích:\n{repo_info}\n\n"
                            "Lệnh đã trích từ dữ liệu:\n{commands}\n\n"
                            "Dữ liệu đã đọc:\n{data}\n\n"
                            "Hãy viết câu trả lời cuối cùng, không nhắc tên tool."
                        ),
                        user_query=request.text,
                        repo_info=chr(10).join(repo_lines) if repo_lines else t("skills.unknown", default="không rõ"),
                        commands=command_text,
                        data=section_text or t("skills.github_no_info", default="không có dữ liệu bổ sung"),
                    )
                ),
            ],
    ...
    def _try_github_selected_repo_followup(self, request: MiaChatRequest, context: MiaContext) -> MiaChatResponse | None:
        ...
        if wants_tree and not (wants_readme or wants_overview or wants_tech):
            ...
            if not result.ok:
                error = result.error or ErrorEnvelope.build(
                    ...
                    user_message=str(result.text or t("skills.github_get_repo_tree_failed", default="Mia chưa đọc được cây repo này.")),
                    request_id=request.resolved_request_id(),
                    thread_id=request.resolved_thread_id(),
                    chat_id=request.chat_id,
                )
        ...
        if wants_readme or wants_overview or wants_tech or wants_tree:
            ...
            if not sections:
                ...
                return MiaChatResponse(
                    ok=False,
                    final_text=t("skills.github_analysis_failed", default="Mia chưa đọc được đủ dữ liệu của repo này để phân tích sâu. Anh thử lại hoặc chọn repo khác nhé."),
        ...
        if wants_branch:
            ...
            if not result.ok:
                error = result.error or ErrorEnvelope.build(
                    ...
                    user_message=str(result.text or t("skills.github_list_branches_failed")),
                    request_id=request.resolved_request_id(),
                    thread_id=request.resolved_thread_id(),
                    chat_id=request.chat_id,
                )
    ...
    def _try_github_search_followup(self, request: MiaChatRequest) -> MiaChatResponse | None:
        ...
        if not selected_row:
            if has_drilldown_cue and parsed_search_rows:
                return MiaChatResponse(
                    ok=True,
                    final_text=t("skills.github_unselected_drilldown"),
        ...
        if explicit_selection and not has_drilldown_cue:
            ...
            return MiaChatResponse(
                ok=True,
                final_text=t("skills.github_repo_selected", repo_name=repo_name),
                tools_called=["memory_search"],
                thread_id=request.resolved_thread_id(),
                request_id=request.resolved_request_id(),
                trace={},
