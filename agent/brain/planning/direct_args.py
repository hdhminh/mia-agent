...
from agent.skills.registry import DIRECT_TOOL_DEFAULT_ARGS
from agent.brain.parsers import (
    normalize_query_text, any_keyword_matches, strip_prefixes,
    strip_conversational_fillers, extract_sheet_range,
    _calendar_range_from_text, extract_github_repo_context,
    extract_news_topic, news_topic_to_feed_slug,
    extract_shortlink_parts, extract_first_url,
)
...
def build_direct_tool_args(
    ...
    if tool_name in {"tasks_list", "tasks_list_due", "tasks_list_overdue"}:
        return {
            "date": str(metadata.get("date") or "").strip(),
            "limit": int(metadata.get("limit") or 20),
            "tasklistId": str(metadata.get("tasklistId") or "@default").strip(),
        }

    if tool_name in {"contacts_search", "contacts_resolve_recipient"}:
        query = strip_prefixes(text, ("contact", "contacts", "danh bạ", "danh ba", "người liên hệ", "nguoi lien he"))
        return {"query": query.strip(), "limit": int(metadata.get("limit") or 10)}
