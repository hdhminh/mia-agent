...
def _feedback_style_payload(verdict: str, comment: str, current_text: str, correction_text: str) -> dict[str, Any] | None:
    ...
    if normalized_verdict in {"shorter", "longer", "deeper", "clearer", "readable"}:
        ...
        return {
            ...
            "prompt_hint": {
                "style_short": "Khi người dùng muốn ngắn hơn hoặc gọn hơn, giữ câu trả lời ngắn, ít bullet, ưu tiên phần quan trọng nhất.",
                "style_deep": "Khi người dùng muốn dài hơn, kỹ hơn, chi tiết hơn hoặc nhiều trang hơn, trả câu trả lời đầy đủ hơn theo bullet rõ ràng và bao quát hơn.",
                "format_readable": "Khi trả lời tài liệu, chia thành tiêu đề ngắn và bullet rõ ràng để dễ đọc trên Telegram; tránh viết thành một đoạn dài.",
            }[topic],
            "memory_hint": {
    ...
    if normalized_verdict in {"incorrect", "down"} or "sai" in joined or "chua dung" in joined or "chưa đúng" in joined:
        if any(token in joined for token in ("anh", "document", "file", "pdf", "tai lieu", "tài liệu")):
            return {
                ...
                "prompt_hint": "Khi người dùng sửa hoặc phản hồi về tài liệu, hãy bám đúng file gần nhất, trả lời theo ngữ cảnh tài liệu và không suy đoán ngoài nội dung.",
        ...
        if any(token in joined for token in ("anh", "image", "anh nay", "ảnh này", "photo", "ảnh", "ocr")):
            return {
                ...
                "prompt_hint": "Khi có ảnh đính kèm và người dùng nói phân tích / mô tả / xem cái này / đọc chữ, ưu tiên image_describe hoặc image_ocr thay vì fallback chung.",
