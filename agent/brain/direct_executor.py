...
from agent.brain.planner import build_direct_tool_args, should_allow_direct_route
from agent.brain.response_normalizer import cap_visible_links, sanitize_final_text
...
def build_memory_recent_text(memory_repo: MemoryRepository, chat_id: str, limit: int = 5) -> str:
    ...
    if not rows:
        return t("skills.direct_memory_empty")

    lines = [t("skills.direct_memory_header")]
...
def _extract_payload_context(payload: dict[str, Any]) -> str:
    ...
    if isinstance(result, dict):
        ...
        if title:
            parts.append(t("skills.doc_label", title=title))
        if text:
            parts.append(t("skills.main_content_label", text=text))
        ...
        if isinstance(result.get("data"), dict):
            ...
            if extra_text and extra_text != text:
                parts.append(t("skills.extra_details_label", text=_normalize_memory_text(extra_text, limit=2500)))
...
def _extract_web_payload_context(payload: dict[str, Any]) -> str:
    ...
    if isinstance(result, dict):
        ...
        if title:
            parts.append(t("skills.title_label", title=title))
        if url:
            parts.append(f"URL: {url}")
        if summary:
            parts.append(t("skills.summary_label", summary=summary))
        if content and content != summary:
            parts.append(t("skills.content_label", content=_normalize_memory_text(content, limit=5000)))
    text_value = str(payload.get("text") or "").strip()
    if text_value and (not parts or text_value not in " ".join(parts)):
        parts.append(t("skills.result_label", text=text_value))
    return "\n".join(parts).strip()
        ...
        *,
        ...
        if not result.ok:
            error = result.error or ErrorEnvelope.build(
                ...
                user_message=str(result.text or t("error.tool_failed")),
                retryable=False,
                request_id=request_id,
                thread_id=thread_id,
        ...
        if hint_tool in {"github_search_repos", "github_list_user_repos"}:
            try:
                ...
                if search_context:
                    ...
                    if not followup:
                        followup = t("skills.github_followup_prompt")
                    if followup:
                        followup = f"\n\n{followup}"
