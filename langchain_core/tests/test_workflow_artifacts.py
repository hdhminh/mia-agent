...
if str(ROOT / "langchain_core") not in sys.path:
    sys.path.insert(0, str(ROOT / "langchain_core"))
...
class TestWorkflowArtifacts(unittest.TestCase):
    ...
    def test_error_monitor_has_chat_id_fallback(self) -> None:
        path = ROOT / "workflows" / "core" / "workflow_error_monitor.json"
        workflow = json.loads(path.read_text(encoding="utf-8"))
    ...
    def test_workflow_validator_accepts_current_gateway_and_monitor(self) -> None:
        ...
        error_monitor = ROOT / "workflows" / "core" / "workflow_error_monitor.json"
