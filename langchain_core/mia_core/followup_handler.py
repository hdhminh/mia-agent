...
from mia_core.prompts import DOCUMENT_FOLLOWUP_CUES, URL_FOLLOWUP_CUES
    ...
    def _try_document_memory_followup(self, request: MiaChatRequest) -> MiaChatResponse | None:
        ...
        for index, row in enumerate(rows, start=1):
            ...
            if row_title and not title:
                title = row_title
            if chunk_text:
                context_lines.append(f"Phần khớp {index}: {chunk_text}")
        ...
        try:
            result, provider_used = self.service._invoke_model_with_fallback(
                ...
                [
                    SystemMessage(
                        content=(
                            "Bạn là Mia. Người dùng đang hỏi tiếp về một tài liệu vừa xử lý gần đây. "
                            "Chỉ dựa trên nội dung tài liệu được cung cấp, trả lời tự nhiên bằng tiếng Việt, "
                            "ngắn gọn nhưng đủ ý. Nếu tài liệu không chứa thông tin cần thiết, nói rõ là chưa thấy."
                        )
                    ),
                    *([SystemMessage(content=learning_guidance)] if learning_guidance else []),
                    HumanMessage(
                        content=(
                            f"Tên tài liệu gần nhất: {title or 'không rõ'}\n\n"
                            f"Câu hỏi tiếp theo của người dùng:\n{text}\n\n"
                            f"Ngữ cảnh tài liệu gần đây:\n{'\n\n'.join(context_lines)}\n\n"
            ...
            error = ErrorEnvelope.build(
                ...
                severity="error",
                message=str(exc),
                user_message="Mia chưa xử lý được câu hỏi tiếp theo của tài liệu này. Bạn thử lại sau nhé.",
                retryable=True,
                request_id=request.resolved_request_id(),
                thread_id=request.resolved_thread_id(),
                chat_id=request.chat_id,
        ...
        try:
            result, provider_used = self.service._invoke_model_with_fallback(
                ...
                [
                    SystemMessage(
                        content=(
                            "Bạn là Mia. Người dùng đang hỏi tiếp về một link vừa xử lý gần đây. "
                            "Chỉ dựa trên nội dung link được cung cấp, trả lời tự nhiên bằng tiếng Việt, "
                            "ngắn gọn nhưng đủ ý. Nếu link không chứa thông tin cần thiết, nói rõ là chưa thấy."
                        )
                    ),
                    *([SystemMessage(content=learning_guidance)] if learning_guidance else []),
                    HumanMessage(
                        content=(
                            f"Tên link gần nhất: {title or 'không rõ'}\n\n"
                            f"Câu hỏi tiếp theo của người dùng:\n{text}\n\n"
                            f"Ngữ cảnh link gần đây:\n{'\n\n'.join(context_lines)}\n\n"
                            "Hãy trả lời trực tiếp cho người dùng."
                        )
                    ),
                ],
            ...
            error = ErrorEnvelope.build(
                ...
                severity="error",
                message=str(exc),
                user_message="Mia chưa xử lý được câu hỏi tiếp theo của link này. Bạn thử lại sau nhé.",
                retryable=True,
                request_id=request.resolved_request_id(),
                thread_id=request.resolved_thread_id(),
                chat_id=request.chat_id,
